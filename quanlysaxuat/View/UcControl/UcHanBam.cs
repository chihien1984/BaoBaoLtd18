...
{
    public partial class UcHanBam : UserControl
    {
        public UcHanBam()
        {
            InitializeComponent();
        ...
        private void UcHanBam_Load(object sender, EventArgs e)//from Load
        {
            LOAD_TRE();
        ...
        private void LOAD_TIME_DMGIAOHANG()
        {
            ...
            gridControl2.DataSource = kn.laybang(" SELECT *  FROM dbo.tblHanBam where  convert(Date, ngaynhan, 103)   between '" + dptu_ngay.Value.ToString("yyyy/MM/dd") + "' and '" + dpden_ngay.Value.ToString("yyyy/MM/dd") + "'");
            kn.dongketnoi();
        }
        private void LOAD_DMGIAOHANG_CODE()
        {
            ketnoi kn = new ketnoi();
            gridControl2.DataSource = kn.laybang("SELECT *  FROM dbo.tblHanBam where IDSP like '" + txtCode.Text + "'");
            kn.dongketnoi();
        }
        private void LOAD_MAGIAOHANG()
        {
            ketnoi kn = new ketnoi();
            gridControl2.DataSource = kn.laybang("SELECT *  FROM dbo.tblHanBam where MaGH like '" + txtMagiaohang.Text + "'");
            kn.dongketnoi();
        }
        private void LOADGridLookupEdit()
        {
            ...
            CbMadhLook.Properties.DataSource = Connect.laybang("select  Donvisp,madh,SPLR,SLSPLR,nvkd,ngaytrienkhai,IDSP,sanpham,Mact,Ten_ct,So_CT,soluongyc,tonkho, "
                      ...
                      + " KetThucTo_HanBam,BTPT_HanBam,TRONGLUONG_HanBam, khachhang, xeploai,MaPo,LoaiDH, Ghichu from tblchitietkehoach where madh is not null and KetThucTo_HanBam is not null "
        ...
        {
            ketnoi Connect = new ketnoi();
            gridControl2.DataSource = Connect.xulydulieu("update tblHanBam set MaGH='" + txtMagiaohang.Text + "' where Num like '" + txtId.Text + "'");
            LOAD_TIME_DMGIAOHANG();Connect.dongketnoi();
        }
        private void HuyMaPGH()
        {
            ketnoi Connect = new ketnoi();
            gridControl2.DataSource = Connect.xulydulieu("update tblHanBam set MaGH ='' where Num like '" + txtId.Text + "' ");
            LOAD_TIME_DMGIAOHANG(); Connect.dongketnoi();
        }
            ...
            try
            {
                if (txtCode.Text != "" && txtKinhdoanh.Text != "" && CbMadhLook.Text != "")/// insert cách truyền tham số parameters
                {
                    ...
                    if (cn.State == ConnectionState.Closed)
                        cn.Open();
                    SqlCommand command = new SqlCommand("INSERT INTO tblHanBam(Mact,So_CT,SL_CTHT,TL_CTHT,Maubv,LoaiDH,MaPo,IDSP,ngaynhan,nvkd,madh,sanpham,"
                    + "chitietsanpham,cdthanhpham,soluongsx,ngoaiquang,mabv,donvi,daystar,dayend,BTPT_HanBam,"
                    + "TRONGLUONG_HanBam,khachhang,xeploai,ghichu,TenBPnhan,MaSQL,TenSQL,Donvisp,SoluongSP)"
                    + "values (@Mact,@So_CT,@SL_CTHT,@TL_CTHT,@Maubv,@LoaiDH,@MaPo,@IDSP,GETDATE(),@nvkd,@madh,@sanpham,"
                    + "@chitietsanpham,@cdthanhpham,@soluongsx,@ngoaiquang,@mabv,@donvi,@daystar,@dayend,@BTPT_HanBam,"
                    + "@TRONGLUONG_HanBam,@khachhang,@xeploai,@ghichu,@TenBPnhan,@MaSQL,@TenSQL,@Donvisp,@SoluongSP)", cn);
                    ...
                    command.Parameters.Add(new SqlParameter("@BTPT_HanBam", SqlDbType.NVarChar)).Value = SoLuongHoanThanh;
                    command.Parameters.Add(new SqlParameter("@TRONGLUONG_HanBam", SqlDbType.Float)).Value = TRONGLUONGTT.Text;
        ...
        private void CAPNHAT_HETHONG()
        {
            try
            {
                ...
                conn.Open();
                DataSet ds = SqlHelper.ExecuteDataset(conn, "[HanBam_UPDATE]");
        ...
        private void XOA_DLSX()
        {
            ...
            {
                ketnoi kn = new ketnoi();
                int kq = kn.xulydulieu("UPDATE tblHanBam SET BTPT_HanBam = 0,TRONGLUONG_HanBam = 0  Where Num ='" + txtId.Text + "'");
                SqlConnection conn = new SqlConnection();
                conn.ConnectionString = Connect.mConnect;
                conn.Open();
                DataSet ds = SqlHelper.ExecuteDataset(conn, "HanBam_UPDATE");
                gridControl2.DataSource = kn.xulydulieu("DELETE tblHanBam Where Num ='" + txtId.Text + "'");
                if (kq < 0)
                { MessageBox.Show("Không Thành Công", "THÔNG BÁO"); }kn.dongketnoi();
            }
            LOADGridLookupEdit(); LOAD_TIME_DMGIAOHANG();
        }
        private void LOAD_PRINT_PHIEUGH()
        {
            DataTable dt = new DataTable();
            ketnoi kn = new ketnoi();
            dt = kn.laybang("select * from tblHanBam where MaGH  like N'" + txtMagiaohang.Text + "'");
        ...
        private void SHOW_TIENDO(object sender,EventArgs e)
        {
            frmPr_BamHan PrBamHan = new frmPr_BamHan();
            PrBamHan.Show();
        }
        private void SHOW_THONGKE(object sender, EventArgs e)
        {
            //frmTK_HanBam TKHanBam = new frmTK_HanBam();
            //TKHanBam.Show();
        }
