...
{
    public partial class UcNhaKyCVT01 : UserControl
    {
        ...
        public void LoadGrid1()
        {
            ...
            gridControl1.DataSource = kn.laybang(" select CTKH.IDSP,nvkd,madh,sanpham,T01.SOLUONGTP,T01.TRONGLUONGTP,soluongyc,tonkho, "
                       ...
                       + " (select IDSP, sum(BTPT01) as SOLUONGTP, sum(TRONGLUONG01) AS TRONGLUONGTP from tbl01 group by IDSP) AS T01 "
                       + " ON CTKH.IDSP = T01.IDSP where madh is not null and dayend is not null");
        }
        public void LoadGrid2()//Hiện dữ liệu đã ghi vào sổ nhật ký công việc
        {
            gridControl2.DataSource = knn.laydulieu("Select * from [dbo].[NKCVT01_DAP] where Ngay between '" + dateTimePickerTU.Value.ToString("MM/dd/yyyy") + "' "
                                                   + "and '" + dateTimePickerDEN.Value.ToString("MM/dd/yyyy") + "' order by Ngay,Hoten,NgayCapNhat ASC");
        }
        ...
        private void AutoCompleteCDSP()// AUTOCOMPLETE CONG DOAN SAN PHAM
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("select Congdoan from [dbo].[tblNHATKY_T01DAP] where  Congdoan is not null", con);
                con.Open();
                SqlDataReader reader = cmd.ExecuteReader();
        ...
        private void SelectSoluongDH()//                                                             Select số lượng đơn hàng theo sản phẩm
        {
            ...
            if (con.State == ConnectionState.Closed)
                con.Open();
            cmd = new SqlCommand("select SLSP from tblNHATKY_T01DAP where Ten_quicach like N'" + txtsp.Text + "'", con);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
        ...
        private void selectMadh_sanpham()//                                                            Select Mã đơn hàng, sản phẩm
        {
            ...
            if (con.State == ConnectionState.Closed)
                con.Open();
            cmd = new SqlCommand("select Madh from tblNHATKY_T01DAP where Ten_quicach like N'" + txtsp.Text + "'", con);
            SqlDataReader reader = cmd.ExecuteReader();
            if (reader.Read())
                txtmadh.Text = Convert.ToString(reader[0]);
            reader.Close();
        }
        private void UcNhaKyCVT01_Load(object sender, EventArgs e)         //FORM LOAD
        {
            ketnoi kn = new ketnoi();
            ...
            cbMaBP.Text = "T01";
            txtmadh.DataSource = knn.laydulieu("select distinct madh from tblchitietkehoach");
            ...
            string DateTu = DateTime.Now.ToString("01/MM/yyyy");
            TimeNow();
            dateTimePickerTU.Text = DateTu;
            dateTimePickerDEN.Text = DateTime.Now.ToString();
        ...
        private void Autocomplete_TenQuicachSP()//Autocomplete tên qui cách sản phẩm
        {
            ...
            using (SqlConnection con = new SqlConnection(ConString))
            {
                SqlCommand cmd = new SqlCommand("select distinct Ten_quicach from NKCVT01_DAP where Ten_quicach != ''", con);
                con.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                AutoCompleteStringCollection MyCollection = new AutoCompleteStringCollection();
                while (reader.Read())
        ...
        private void btnsave_Click_1(object sender, EventArgs e)
        {
            if (txtHOTENNV.Text != "")
            {
                ...
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand("INSERT INTO [dbo].[tblNHATKY_T01DAP] "
        ...
        private void btnsua_Click_1(object sender, EventArgs e)
        {
            if (txtHOTENNV.Text != "")
            {
                ...
                {
                    con.Open();
                    using (SqlCommand cmd = new SqlCommand("UPDATE [dbo].[tblNHATKY_T01DAP] SET "
                        ...
                        + " ChuyenGiao=@ChuyenGiao,NgayCapNhat=GetDate() WHERE IdenKey LIKE '" + txtIdekey.Text + "'", con))
                    {
                        ...
                        /* gridControl2.DataSource = knn.laybang("SELECT Code,Ngay,Manv,Hoten,Ten_quicach,SLSP "
                                         ...
                                         + ", IdenKey, GIOLAM, CONG_SUAT_GIO FROM [dbo].[NKCVT01_DAP]  "
        ...
        private void btnxoa_Click_1(object sender, EventArgs e)
        {
            ...
            {
                ketnoi kn = new ketnoi();
                int kq = kn.xulydulieu("delete [dbo].[tblNHATKY_T01DAP] where IdenKey like '" + txtIdekey.Text + "' ");
                ketnoi knn = new ketnoi();
                LoadGrid2();
                gridView2.FocusedRowHandle = gridView2.RowCount - 1;
                /*gridControl2.DataSource = knn.laybang("SELECT Code,Ngay,Manv,Hoten,Ten_quicach,SLSP "
        ...
        private void btnthongke_Click(object sender, EventArgs e)
        {
            gridControl3.DataSource = knn.laydulieu("select Ngay,Hoten,Manv,Congdoan,Madh,Ten_quicach, "
                                     ...
                                     + " CONG_SUAT_GIO, PHUT_GOMTGNGHI from [dbo].[NKCVT01_DAP] "
        ...
        private void txtcodechange_TextChanged(object sender, EventArgs e)
        {
            ...
            gridControl2.DataSource = kn.laybang("SELECT Code,Ngay,Manv,Hoten,Ten_quicach,SLSP "
                                        + ", SLCD, Congdoan, Madh, SL_SanXuat, GioLam_BatDau "
                                        + ", GioLam_KetThuc, MaBP, TenBP, ChuyenGiao, NgayCapNhat "
                                        + ", IdenKey, GIOLAM, CONG_SUAT_GIO FROM NKCVT01_DAP where Code like '" + txtcodechange.Text + "'");
            ...
            //                             + " from NKCVT01_DAP where Code like '" + txtIDSP.Text + "'");
            //gridControl3.DataSource = knn.laydulieu("select NK.Code,Ten_quicach,NK.Congdoan,SOLUONG_HT from tblNHATKY_T01DAP NK, "
            // + "(select Code, Congdoan, SUM(SL_SanXuat) as SOLUONG_HT from tblNHATKY_T01DAP group by Code, Congdoan) NK_T01 "
            // + "WHERE NK.Code = NK_T01.Code");
        }
        ...
        private void txtmadh_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            gridControl1.DataSource = kn.laybang(" select CTKH.IDSP,nvkd,madh,sanpham,T01.SOLUONGTP,T01.TRONGLUONGTP,soluongyc,tonkho, "
                       ...
                       + " KetThucTo1, khachhang, xeploai, Ghichu from tblchitietkehoach CTKH left join "
                       + " (select IDSP, sum(BTPT01) as SOLUONGTP, sum(TRONGLUONG01) AS TRONGLUONGTP from tbl01 group by IDSP) AS T01 "
                       + " ON CTKH.IDSP = T01.IDSP where madh is not null and dayend is not null and madh like N'" + txtmadh.Text + "'");
        }
    }
}
